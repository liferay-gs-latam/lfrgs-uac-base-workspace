...
public class CMICUserDisplay {
	...
	public CMICUserDisplay(User user, String portraitImageURL, CMICUserDTO cmicUserDTO) throws PortalException {
		if (user != null) {
		...
		if (cmicUserDTO != null) {
			_producer = cmicUserDTO.isProducer();
			_cmicUserId = cmicUserDTO.getId();
			_uuid = cmicUserDTO.getUuid();

			List<CMICUserRelationDTO> organizations = cmicUserDTO.getOrganizations();

			List<CMICUserRelationDisplay> cmicOrganizationDisplayList = new ArrayList<>();

			for (CMICUserRelationDTO organization : organizations) {
				cmicOrganizationDisplayList.add(new CMICUserRelationDisplay(organization));
			}

			_organizationList = cmicOrganizationDisplayList;

			List<CMICUserRelationDTO> accounts = cmicUserDTO.getAccounts();

			List<CMICUserRelationDisplay> cmicAccountDisplayList = new ArrayList<>();

			for (CMICUserRelationDTO account : accounts) {
				cmicAccountDisplayList.add(new CMICUserRelationDisplay(account));
			}

			_accountList = cmicAccountDisplayList;
		}
	}

	public List<CMICUserRelationDisplay> getAccountList() {
		return _accountList;
	}
	...
	private List<CMICUserRelationDisplay> _accountList = new ArrayList<>();
